...
## a)
The data to be used in this assignment is `windshieldy1`.
This data includes the values for the observed hardness values $y_1$ of a sample of windshields.
```{r}
library(aaltobda)
data("windshieldy1")
head(windshieldy1)
...
We assume the data to be normally distributed $\text{N}(\mu, \sigma^2)$ with an unknown standard deviation $\sigma$ and an unknown average hardness $\mu$.
Assuming prior independence of location and scale parameters, we know that the prior distribution is
$$p(\mu, \sigma^2)\propto (\sigma^2)^{-1}.$$
The likelihood can be infered to be 
$${\displaystyle {\begin{aligned}p(y \mid \mu ,\sigma ^{2})&=\left({\frac {1}{2\pi \sigma ^{2}}}\right)^{n/2}\exp \left(-{\frac {1}{2\sigma ^{2}}}\sum _{i=1}^{n}(y_{i}-\mu )^{2}\right)\end{aligned}}}.$$
Under the aforementioned prior, the joint posterior distribution is proportional to the likelihood function, resulting in
$${\displaystyle {\begin{aligned}p(\mu ,\sigma ^{2} \mid y)&=\sigma^{-n-2}\exp \left(-{\frac {1}{2\sigma ^{2}}}[(n-1)s^2+n(\bar{y}-\mu)^2]\right)\end{aligned}}}.$$
...
As was suspected from the predictive interval, this density function is much wider than the one seen before.
Our confidence in this prediction is lower than in the previous.


# Exercise 2) Inference for the difference between proportions
## a)

We have an experiment with two groups: control and treatment.
The probability of death with control is $p_0$ and the probability of death with the treatment is $p_1$.
...
$$p(y_i|p_i)=\begin{pmatrix}n_i\\y_i\end{pmatrix}p_i^{y_i}(1-p_i)^{n_i-y_i}.$$
...
based on the derivation of the posterior in p.35 of the BDA3.
Therefore, the resulting posteriors are $p(p_0|y)=\text{Beta}(p_0|40, 636)$ and $p(p_1|y)=\text{Beta}(p_1|23, 659)$.
...
The two prior distributions were chosen so that it is equally likely that the treatment is effective as it is not.
The point estimate for posterior distribution of the odds ratio is 0.57 and the 0.95 posterior interval is [0.32, 0.93].
Assuming priors $p_0$ and $p_1$, there is a 95% probability that the true treatment effect is in the interval [0.32, 0.93].
Furthermore, there is a 57% probability that the treatment results are different from those in the control group.
...
We can see the results with different priors are plotted in different colors.
In this case, the posterior distributions for $p_0$ and $p_1$ do not seem to be very sensitive to the choice of prior.
Thus, the uniform prior seems to be a sensible choice for this case given the amount of data we have.

# Exercise 3) Inference for the difference between normal means
## a)

In this case, there are two production lines with hardness measurements $y_1$ and $y_2$ as oppose to the one in exercise one.
The prior, likelihood and posterior follow the same distributions as in Exercise 1.

The prior distribution is
$$p(\mu_i, \sigma_i^2)\propto (\sigma_i^2)^{-1}$$
with $i=1,2$.
The likelihood can be infered to be 
$${\displaystyle {\begin{aligned}p(y_i \mid \mu_i ,\sigma_i ^{2})&=\left({\frac {1}{2\pi \sigma_i ^{2}}}\right)^{n/2}\exp \left(-{\frac {1}{2\sigma_i ^{2}}}\sum _{j=1}^{n}(y_{i}^{j}-\mu_i )^{2}\right)\end{aligned}}}.$$
The posterior follows the standard $t$ density $p(\mu_i \mid y_i)\sim t_{n_i-1}(\bar{y_i}, s_i^2/n_i)$ with $n_i-1$ degrees of freedom.
...
Given the priors that were chosen, the point estimate for $\mu_d$ is $-1.21$ with 95% posterior intervals $[-1.78 -0.63]$.

## b)
Assuming the priors $p_1$ and $p_2$, there is more than a 95% probability that the means are different as the posterior intervals show.
Given that the upper value of the interval is inferior to 0, it is very likely that the values of the means differ.
